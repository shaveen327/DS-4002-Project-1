...
physical_sorted <- physical_conditions %>%
  ...
  # Sort the data by 'Number.of.Beneficiaries' in ascending order
...
# Summarize total beneficiaries to top 25
...
# Summarize total beneficiaries by state to select top 25 states
top_states <- condition_summary %>%
  group_by(State) %>%
  summarise(Total_Beneficiaries_State = sum(Total_Beneficiaries)) %>%
  arrange(desc(Total_Beneficiaries_State)) %>%
...
# Create the plot for the fifth condition in the top 25 states
